...
For self-deployed SeaweedFS TLS runs, `mlflow-tests/images/test-run.sh` extracts `ca-bundle.crt` from the configured CA ConfigMap and exports it through the standard Python, requests, curl, and AWS CA environment variables. Preserve that trust setup for presigned artifact downloads; do not replace it with disabled certificate verification.
